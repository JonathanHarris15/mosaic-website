...
/* ⚠ EVERY SELECTOR BELOW IS SCOPED TO .m-header, AND TWO OF THEM ONCE WERE
   NOT. .m-back__label and .m-btn__label are slots, not header parts — a
   Button anywhere on the page carries one. Written bare inside this query they
   hid the word out of EVERY button in the document under 640px, so on a phone
   the app drew rows of blank tan pills with no label and no icon in them: the
   page still worked and simply stopped saying what any of its buttons did.
   Collapsing a control to a square glyph is a thing a HEADER does to make room
   for the title; it is never a thing to do to the page (MS-674). */
@media (max-width: 640px) {
  ...
  .m-header__sub, .m-header__crumbs, .m-header__rule { display: none; }
  .m-header .m-back__label, .m-header .m-btn__label { display: none; }
  .m-header .m-back { min-width: 44px; justify-content: center; padding-right: 0; }
...
`,
...

  {
    ...
    notes: [
      ...
      "⚠ `--open` IS A WEEK, NOT A DAY. A cell cannot be taller than its row, so opening one opens the row — and every day on that row then shows everything, rather than sitting beside empty space with events still hidden. `__grid--open` stops that grid dividing its space equally; the caller pins every cell in it to a height in pixels, so the opened week is the only one that moves and the growing is something you can watch.",
      "⚠ A ROW HEIGHT BELONGS TO THE MONTH IT WAS MEASURED IN. Where several months are laid out at once, they do not all span the same number of weeks — six weeks divide the same box into six rows, five into five. Measure the wrong one and every row comes out proportionally wrong, which reads as the whole month jumping rather than one row growing. `__grid--open` is per-grid for the same reason.",
...
/* ── A week opened out ────────────────────────────────────────
   ...
   ⚠ align-content: start IS LOAD-BEARING, NOT TIDINESS. Auto rows
   SHARE OUT any height the grid box has beyond what its rows need,
   and the box is not the one deciding how tall it is — open, the
   page stops fitting the window and the taller of the two columns
   sizes the row, which is the right rail as often as not. Left to
   stretch, every one of the five rows took a share of the surplus
   while the cells stayed pinned to their pixel height, so all seven
   days of all five weeks grew a gap under their chips at once. One
   row was asked to move and the whole month moved. */
.m-cal--fit .m-cal__grid--open { grid-auto-rows: auto; align-content: start; }
/* Pixels, because a height has to be a number at both ends for the
   ...
   ⚠ THIS RULE ONLY WORKS BECAUSE --duration-slow IS DECLARED IN THE
   SHEET THAT SHIPS IT. Motion used to be written by hand in the
   design system's spacing.css, below the generated marker, where
   nothing carried it into mosaic.css — and a var() that resolves to
   nothing does not fall back, it deletes the declaration it sits in.
   This transition never ran once. Motion is generated from
   tailwind.config.js now; test/calendar-week-open.test.js is what
   stops it drifting back out of reach. */
