...
/* Pixels, because a height has to be a number at both ends for the
   ...
   ⚠ LITERALS, NOT var(--duration-slow) / var(--ease-standard). Those
   two are the design system's own, declared after the @generated
   marker in spacing.css, and nothing splices them into mosaic.css —
   so they resolve to nothing, the whole declaration goes with them,
   and the row jumps to its open height instead of travelling there.
   Same trap as the note on .rm-add in roles-manager.html. The
   300ms is --duration-slow; the curve is --ease-standard. */
.m-cal--open .m-cal__cell { transition: height 300ms cubic-bezier(0.2, 0, 0, 1); }
