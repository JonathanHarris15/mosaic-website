...
//
// Unlike the member-tag sync (add-only), the Elder Tag is a Projected
// Tag: it is kept in exact sync with the source of truth, so it is BOTH
// added when a linked user becomes an elder AND removed when they stop
...
/**
 * True only for the elder level — super_admin is excluded (a distinct
 * office).
 * @param {string} permissionLevel the user's permission level
 * @return {boolean}
 */
 ...
 * @param {Array} personTags the person's current tags
 * @return {boolean}
 */
...
/**
 * Should the trigger ADD the Elder tag? Iff the linked user is an elder and
 * the person doesn't already carry it. The tag-absent check makes the write
 * a no-op once synced, so the trigger doesn't re-fire itself in a loop.
 * @param {string} permissionLevel the user's permission level
 * @param {Array} personTags the person's current tags
 * @return {boolean}
 */
...
/**
 * Should the trigger REMOVE the Elder tag? Iff the linked user is NOT an
 * elder (role changed away, or no link) yet the person still carries it.
 * This is the half the add-only member sync deliberately omits — the Elder
 * Tag is projected, so a stale tag must be cleared.
 * @param {string} permissionLevel the user's permission level
 * @param {Array} personTags the person's current tags
 * @return {boolean}
 */
