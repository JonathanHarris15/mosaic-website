...
**Note Body**:
The rich-text content of anything written in the Note Module — a Shepherding Note, Meeting Minutes, an [[Elder Document]] or an [[Event Document]]. TipTap JSON in the editor, everywhere, whatever it hangs off. **Stored as one TipTap JSON value everywhere except an Elder Document**, which stores its body as [[Block]]s so it can be saved a paragraph at a time (ADR-0065). Exports to Word and imports from it by a walk this codebase owns (ADR-0048). May hold headings, bold/italic/underline, highlights, fonts and sizes, bullet and numbered lists, tables, alignment, links and pictures — a picture rides INSIDE the body as a data URI rather than in Storage, so whatever rule governs the document governs the picture. May contain Cross-References, but only where the surrounding document is elder-only — see Note Module.
_Avoid_: Content, text, body (use Note Body as the full compound term)

**Block**:
One heading, paragraph, code block, quote, list, list item, table, row, cell, picture, rule or [[Person Panel]] of an [[Elder Document]]'s Note Body, stored as its own field `blocks.<id>` (ADR-0065). Its id lasts: typing, Enter and paste never hand one id to two blocks. It knows the block it sits inside (`parent`) and its place among its siblings (`order`, a key that can always fit another between two). A save writes only the Blocks that changed, so two elders in two paragraphs never undo each other. A heading, paragraph, code block, list item, cell or Person Panel is a box in the [[Box lock]]; a list, quote, table or row is only structure.
_Avoid_: Node (ProseMirror's word for what is on screen), section, chunk
...
- **Fields**: `title`, `blocks` (the Note Body as [[Block]]s; a document from before MS-501 holds `contentJson` instead until it is converted — once, the first time it is opened or written, or by the migration script), `authorName`, `authorUid`, `createdAt`, `updatedAt`, `updatedByName`, `docType` (optional, defaults to 'note'), `filterId` / `filterConfig`, `careListColumns` and `careListData` (for care-list type; one field per cell, person then column), `ownerPersonId` and `inLibrary` (profile scope only).
...
### Document Library
The collection of all Elder Documents together with their folder organisation. Consists of two things in Firestore: the flat `elder_documents` collection (one doc per document, keyed by ID) and a single `elder_document_structure` document that encodes the full folder tree. The tree node carries folder names, child folder nodes, and ordered lists of document IDs — it does not duplicate document content. **A tree is changed one change at a time, on the server** (MS-493): create, rename, move, remove, file or prune, applied to the latest tree inside a transaction — the same path for the Library page, a profile's Documents tab, the phone and the assistant. Nobody writes back a whole tree they loaded, because two elders filing at the same moment would lose one of the changes.
...
A Person Panel's words are saved straight to its Shepherding Note, a moment after typing stops. The panel **follows its note live**: words written on the profile, on the phone or in another document arrive while nobody on this page is typing in it (MS-506).
...
### Person Panel
An embedded block inside an Elder Document's TipTap editor that is linked 1-to-1 with a single Shepherding Note. Inserting a Person Panel creates (or links) the Shepherding Note immediately. Editing the panel's body saves to that note; editing the note on the person's profile shows in the panel while the document is open. **Its box is its note's box**: while somebody has the note open, on the profile or in another document, the panel is read-only and shows their face, and the other way round. When its note is deleted the panel becomes a bold "name — note type" line and the note's words, written as [[Block]]s so every open copy of the document shows it. The panel's header shows the Person's name and is editable — changing it re-associates (moves) the linked note to the newly selected Person.
...
**Live updating**:
The **Services** page, the **Order of Service**, the **Shepherding Profile**, **Tasks & Reminders**, the [[Care List]] (its cells, title, columns, and who is on it), the [[Form Document]] and the [[Elder Document]] (its [[Block]]s, its title and its Person Panels' notes) listen to Firestore for as long as they are open, rather than reading once on load. Inside the phone app a listen stream that stays silent for five seconds is replaced by re-reading every few seconds — the page cannot tell the difference, and a stream the WebView refuses no longer leaves it blank. A change to a field nobody on this screen has touched is simply adopted; a field this editor has changed is left alone until it saves — which needs no merge and asks nobody a question, because the only case that could need a decision is the one the [[Box lock]] prevents. An adopted value moves the **saved snapshot** with it, or the next save reads it as a local edit and writes it straight back, turning something received into something claimed. A snapshot landing mid-edit never rewrites the box under somebody's hands. On the Order of Service every inline editor hides its cell and puts an input in its place, so **a hidden cell is somebody editing** — a signal that cannot drift out of step with what is on screen; a [[Care List]] cell and a [[Form Document]] question are edited in place instead, so those pages remember which box the cursor is in and hand over what arrived when it leaves. A [[Care List]] saves **one field per cell**, and a column change is one change applied to the latest column list, so two elders' cells and column edits never undo each other; a row that leaves the list's filter while somebody is typing in it stays until they leave the cell.
...
**It saves one answer at a time, and updates live** (MS-431). Each answer is its own field and a save writes only the answers that changed — the whole answer is the unit, so a picked person's id and name, or a select-all list, travel together. Two elders on two questions never overwrite each other; each question that asks something, and the title, is a [[Box lock|box]]. Where a [[Personal shepherding document]] is filed is **settled from the stored record**: after saving who it is about, the writer (page or assistant) asks the stored document whether its subject and its filing agree and moves it if not — a step that can be repeated, so two writers settling at once end in the same place, and a move that was missed is repaired by the next save or opening.
