...
**Pastoral Assistant**:
A grant on a User account (`users.pastoralAssistant`), stacked on their [[Permission Level]] — not a Permission Level itself ([ADR 0065](docs/adr/0065-a-pastoral-assistant-is-a-grant-on-the-account.md)). A Pastoral Assistant reads everything an Elder reads (including the editor-rung things an elder sees because an elder is an editor) and writes the record: Elder Documents of every kind, Folders, Shepherding Notes, and [[Task]]s, with the same freedom elders have over those. **MS-594 (Jonathan product override, 2026-09-19):** they also have elder *software powers* for shepherding decision/write actions — they can set [[Shepherding Status]], change tags and tag vocabulary, set [[Elder Assignment]], add an [[Explanation]], manage [[Filtered View]]s and Relationships, move the [[Membership Track]], and write or text [[Prayer Request]]s. They are still **never counted as an Elder**: no [[Elder Tag]], not in elder pickers, no [[Elder Digest]], not Task-assignee-as-elder. They cannot lock guidance, shut a form to elders, or answer an elder-rung form. Only an admin or super admin can grant it.
_Avoid_: secretary, scribe, assistant elder, pastoral_assistant as a Permission Level

**Shepherding System**:
The set of features accessible to Elders, super admins, and a [[Pastoral Assistant]]. Surfaces in the UI as the "Shepherd Landing Page."
...
**Shepherding Note**:
A typed, dated record attached to a Person's Shepherding Profile. Has a Note Type, an optional Subject Line, and a Note Body. Written by an Elder or a [[Pastoral Assistant]]. Visible to all Elders, super admins, and Pastoral Assistants. Any of them can edit or delete any note.
...
**Shepherding Profile**:
The shepherding view of an existing Person record, open to Elders, super admins, and a [[Pastoral Assistant]]. Three tabs: the Pastoral Record for that Person, the documents kept on their profile, and the [[Task]]s the elders have to do for them — the ones carrying them as [[Subject]] ([ADR 0061](docs/adr/0061-a-task-may-name-the-person-it-is-for.md)). The underlying Person is created and managed in the People Manager; the Shepherding Profile layers on top of it.
...
**Status Change**:
A Pastoral Record entry that captures a transition in a Person's Shepherding Status. Records the previous and new status values, which Elder made the change, the source (Shepherding Profile, People list, or Elder Document), and an optional Explanation. Generated automatically whenever a Shepherding Status is set or cleared. An Elder, super admin, or [[Pastoral Assistant]] may set or clear a status (MS-594).
_Avoid_: Status update, status event, status history entry

**Tag Change**:
A Pastoral Record entry that captures a Shepherding Tag being added to or removed from a Person. Records the tag name, whether the action was an addition or removal, which Elder made the change, the source, and an optional Explanation. Generated when tags are changed from the Shepherding Profile or the People list. An Elder, super admin, or [[Pastoral Assistant]] may change tags (MS-594).
_Avoid_: Tag event, tag history entry

**Explanation**:
An optional plain-text annotation that any Elder, super admin, or [[Pastoral Assistant]] can add to a Status Change or Tag Change entry after the fact. Records the reason or context for the change. Distinct from a Shepherding Note — an Explanation is always scoped to a specific Status Change or Tag Change and carries no rich-text editor.
_Avoid_: Note (use Shepherding Note for standalone records), comment, reason

**Filtered View**:
A shared, elder-configured saved filter over the People list that appears as a table widget on every elder's Shepherd Landing Page. Any elder, super admin, or [[Pastoral Assistant]] can create, edit, or delete one (MS-594).
_Avoid_: Custom table, saved search, widget

**Task** — MS-79:
A piece of work an elder has to do, living on the [[Tasks & Reminders]] page and summarised on the Shepherd Landing Page. Carries a title, a rich body, a **due date** with an optional time, its **Assignees**, and optionally a [[Subject]] — the one Person it is *for*. Any elder, super admin, or [[Pastoral Assistant]] can write, edit, tick, skip or delete any Task, including one that is somebody else's — elders are few and trust each other, and a Task locked to a man on holiday is worse than an accidental tick. Replaces the former **Follow-up Reminder**, which was a title, a date and nothing else.
...
**Reminder**:
Not a second entity. The everyday word for a [[Task]] that is just a date with nobody's name on it — which is why the page is called Tasks & Reminders and the elders are not wrong when they say it. There is one thing on the page, and its shape is the same whether or not anybody has picked it up.
_Avoid_: treating it as a type, a flag, or a separate collection

**Assignee**:
An Elder responsible for a [[Task]]. A Task can have **any number, including none** — and none is a real, visible state meaning nobody has picked this up, not a blank. Only Persons carrying the [[Elder Tag]] can be assigned: the page is shepherding-only, so assigning anybody else hands out work they cannot see. A [[Pastoral Assistant]] is never an Assignee — they do not carry the Elder Tag, and they are not counted as an elder.
...
**Elder Tag**:
The Projected Tag that marks a Person as an Elder. Projected from the Person's Linked User having Permission Level `elder` — it is **not** hand-applied: linking/unlinking a User, or a Permission Level change to/from `elder`, adds or removes it. A [[Pastoral Assistant]] grant does **not** project it: the tag still reads only the `elder` level, so a Pastoral Assistant is never tagged and never appears in the assignable-elder set. Like all Projected Tags it is immutable (no rename/delete/merge/hide). It is the canonical answer to "which Persons are Elders," and therefore supplies the assignable-elder set for Elder Assignment. Every Elder is also a Member, so an elder Person carries both the Elder Tag and their Membership Tags. Distinct from the `elder` Permission Level, which is the permission source the tag is projected from (the Permission Level grants the officers' shepherding *decisions*; the tag makes elder-ness a filterable, graph-visible Person attribute). Its visibility in the member-facing Membership Directory is **fixed and visible to ordinary members** (not toggleable, like all Projected Tags) — eldership is a public office.
...
- **A Name Fix is entered in parts.** It asks for a first name, a last name, and an optional suffix, with the same pass for a person who has no last name. The blanks open with the parts already remembered, or with a faithful reading of the full name when none are remembered yet. Approval writes the full name and remembers the parts. A Name Fix that was filed as one full name stays that spelling: it is not taken apart, and an older split does not stay beside the new spelling ([ADR 0070](docs/adr/0070-a-name-fix-is-entered-in-parts.md), [ADR 0074](docs/adr/0074-an-unsplit-name-is-taken-apart-only-when-the-parts-say-the-same-name.md)).
...
**Person name**:
A Person's name is one full name. It is entered as a first name, a last name, and an optional suffix. When a last name was entered, that is what a [[Household]] is called after. A suffix is part of the name and is never the household's name. A full name that was never entered in parts is taken apart when those parts say the same name: a trailing suffix is not a last name, words before the last stay in the first name, and a single word is left as one name. A name that cannot be taken apart that way stays one full name, and a Household suggested from it still uses the last word until the parts are entered. A person who has no last name is still a person — nothing refuses them for lacking one ([ADR 0069](docs/adr/0069-a-persons-name-is-entered-in-parts.md), [ADR 0074](docs/adr/0074-an-unsplit-name-is-taken-apart-only-when-the-parts-say-the-same-name.md)).
_Avoid_: given name, surname (say first name and last name)

**Household**:
A named collection of people who belong together at the foyer — "The Harris Household" might be a married couple, their children, and a grandmother. When a last name was entered, or remembered by taking a full name apart, the name is informed by that last name, not by a suffix. A stored Household keeps its own name either way. A full name that could not be taken apart still suggests from its last word. Distinct from [[Family]], which is the kinship tree (husband, wife, children) and is not stretched to fit whoever walked in together. Stored in its own `households` collection ([ADR 0043](docs/adr/0043-households-are-stored-as-their-own-collection.md)). A Person may belong to more than one Household (a child of separated parents).
...
**Relationship**:
A connection among Persons, surfaced on the Shepherding Profile's Relationships panel (never in the member-facing Membership Directory). Elder-gated, **except** for Types an elder has made a [[Shared Relationship Type]] — see ADR 0017; the panel itself remains closed below elder (a [[Pastoral Assistant]] can read it and, since MS-594, can edit it). Two kinds share the panel: a **Custom Relationship** (elder-authored, stored, deletable — a **Pairwise Relationship** or a **Relationship Group**) and a **Projected Relationship** (derived from Family). Elder Assignment is deliberately **not** shown in this panel — it is a relationship in the Relations Viewer graph only. Relationships can cross-cut households.
...
**Prayer Request**:
The specific thing a pastoral-prayer subject asks the church to pray about for a given Sunday. Captured against that Sunday's Pastoral Prayer (i.e. attached to the order of service) and, once captured, also recorded as a Shepherding Note of Note Type "Prayer Request" on the subject's Shepherding Profile (generated once, then independent). Three ways in: typed in by an Elder or super admin; supplied by the subject themselves by replying to an automated text message; or written on a page reached through an [[Answer link]] in that text, with no account needed. A subject's own later change on the page may refresh the generated note only while it still holds exactly the text it was generated with — Elder edits always win, and there is never a second generated note ([ADR 0007](docs/adr/0007-prayer-request-one-time-generation.md)). Visible to Elders, super admins, and a [[Pastoral Assistant]]; an Elder, super admin, or Pastoral Assistant may write one or send the text (MS-594).
...
**Elder Digest**:
A text message sent to everyone carrying the Elder tag once every pastoral-prayer subject for a Service has a filled Prayer Request, summarising who is being prayed for, the service date, and each request. Sent only when the request that completed the set arrived as a texted reply **or a page answer through an [[Answer link]]** — if an Elder fills the last one in by hand, no digest goes out, since they are already seeing the requests. Its purpose is to surface requests that no Elder was watching for. A [[Pastoral Assistant]] does not carry the Elder Tag and is not sent the digest.
_Avoid_: elder alert, prayer summary, notification (unqualified)

**Answer link**:
A personal, single-purpose, expiring link that lets somebody with no Mosaic account answer one thing through one closed server door ([ADR 0067](docs/adr/0067-an-answer-link-is-a-personal-expiring-door.md)). URL shape `/a/<token>`. The token is 128 bits of randomness in base58, minted server-side only when a text that needs an answer is sent, and stored as a SHA-256 hash — the raw token is never kept. A link names one purpose, one Person, and one thing; the server takes those off the link document, never from the caller. It has no state beyond its expiry — whether it can still do anything is read from the thing it points at, every call. First purpose is `prayer_request` (MS-247); MS-249 will add `trade_invitation`. A forwarded link hands over only what that one link can do.
- **Not a Firestore rule opened to signed-out visitors** ([ADR 0031](docs/adr/0031-the-directory-asks-for-an-account.md)). The signed-out path is one callable, `answerLink`, the same v2 `onCall` shape as `publicForm` ([ADR 0051](docs/adr/0051-a-public-form-is-served-and-answered-through-one-closed-door.md)). `firestore.rules` stays shut on `answer_links`.
- **It does not rely on App Check.** Rate limits hold the door: 10 saves an hour per link, 30 unknown lookups an hour per hashed address. Unknown and expired tokens get the same closed message.
- **Expiry is per purpose, with a 21-day hard cap.** For a Prayer Request the purpose supplies the end of the service date in church-local time.
_Avoid_: magic link, signed URL, public token, form link (that is a public form's `/f/<token>`)
...
- **Fields**:
  ...
  - `nameParts`: Present when the name was entered in parts, or when an existing full name was taken apart because the parts say that same name. Holds the first name, the last name, the suffix, and whether the person has no last name, so the blanks can be reopened and a Household can be told the last name. Not a second name. Absent when the name could not be taken apart that way ([ADR 0069](docs/adr/0069-a-persons-name-is-entered-in-parts.md), [ADR 0074](docs/adr/0074-an-unsplit-name-is-taken-apart-only-when-the-parts-say-the-same-name.md)).
...
- **An [[Event Announcement]] on a one-off lives on that occurrence**, because the occurrence is the event. A date of a repeating event can read the event's announcements and cannot change them.
...
### Event Announcement
What the church should hear about an [[Event series]] or a one-off Event: a title, a short piece of plain prose, and exactly one way of going out. Printed in the Sunday service guide a whole number of weeks ahead, or told to the people who carry every chosen tag. A printed note and a tell are two announcements. The words belong to the event — a repeating event keeps them on the series, a one-off keeps them on its occurrence — and a date of a repeating event can read them and cannot change them. How it goes out, and who would be told, is not part of the words someone who can see the event may read. When the guide for a Sunday is opened, a printed announcement whose dates fall in its weeks is shown beside that Sunday's typed announcements, and it is not saved as one of them. A repeating announcement appears once. A skipped date contributes nothing. A date that was moved counts from the day it was moved to. The handed-out guide names only an event anyone can already see, and the Sunday Service is one of those, whoever is holding the copy.
...
- `elder` — elders, super admins, and a [[Pastoral Assistant]].
...
### Event Announcement
Prose that belongs to an [[Event]] and is announced in one way: told to a list of people, or printed in the Sunday service guide. An Event may carry several. Distinct from the announcements an editor types onto one Sunday — those stay that Sunday's own words. An Event Announcement is read beside them and is never copied into them.
- **It belongs to the Event, not to one date of it.** A one-off carries its own, edited on its event page. A repeating Event carries them on the [[Event series]], edited where the series' other standing facts are edited, and a date reads them through and cannot change them. Same rule as the description: one date decides who is serving; the Event decides what is true every time.
- **One way each.** Told, or printed. Doing both is two announcements.
- **Told is a [[Notification]].** The editor chooses who and when, never the channel — a push if that Person's [[Linked User]] has the app, a text otherwise ([ADR 0036](docs/adr/0036-a-notification-picks-its-channel-and-delivery-is-best-effort.md)). Who is a tag filter, the directory's own: a Person must carry every chosen tag, and at least one tag is required. When, on a one-off, is one or more dates each with a time. When, on a repeating Event, is how many days before each occurrence, at a time. A time outside the Notification's hours is refused. A skipped date tells nobody. A moved date counts from the day it actually happens.
- **Printed** asks how many weeks in advance, and is included on every Sunday that falls on the event's date or within that many weeks before it. A skipped date prints nothing. A moved date counts from its new day.
- **A change affects what has not happened yet.** Guides not yet read show the new prose; a tell already sent is not recalled and is not sent again. Deleting stops the rest.
- **An editor writes them. Anyone who can see the Event can read the prose.** The list of who will be told is editor-only.
_Avoid_: weekly announcements, Sunday booklet text (typed onto one Sunday), blast, and **text** for the tell itself — a text is one channel of a [[Notification]]
...
- **Fields**: `title`, `contentJson`, `authorName`, `authorUid`, `createdAt`, `updatedAt`, `updatedByName`, `docType` (optional, defaults to 'note'), `filterId` / `filterConfig`, `careListColumns` and `careListData` (for care-list type; one field per cell, person then column), `ownerPersonId` and `inLibrary` (profile scope only).
- `docType`: Can be `'note'` (standard TipTap document) or `'care-list'`.

### Author
The Elder or [[Pastoral Assistant]] an Elder Document is written by, recorded as `authorUid` and `authorName`. **Every Elder Document has one**: a document whose author cannot be resolved is refused rather than written, because an untraceable pastoral record is worse than a create that failed — it exists, it stands in the Pastoral Record, and nothing surfaces the problem. Distinct from `updatedByName`, which is whoever touched it last.

### Care List
A type of Elder Document that displays a filtered list of people. The first column shows the person's name (sticky); additional elder-defined columns scroll horizontally and allow editing person attributes inline — including applying or removing Shepherding Tags (`#` / `-#` triggers) and setting the Shepherding Status (`$$` trigger, which spawns the status matrix inline). Status changes made via `$$` generate a Status Change on the person's Pastoral Record. A filled cell **is shown on that person's Shepherding Profile**, in their Pastoral Record, as a read-only entry that links back to the Care List. It is not copied there and not a Shepherding Note: it lives in the Care List and is edited only there, so an empty cell shows nothing and a changed cell shows its new text.
...
All elders, super admins, and a [[Pastoral Assistant]] have equal create/read/update/delete access to all Elder Documents and the folder structure. The shared access core's *writes the record* answer (`isElder()` or the Pastoral Assistant grant) covers the entire Document Library — no per-author restrictions.
...
**Box lock**:
One person per box, refused at the door ([ADR 0035](docs/adr/0035-one-person-per-box.md)). A box another editor holds does not open — it has no editor to click into, and carries their face and first name instead. Two people are therefore never in one box, so there is never a version to choose between; the conflict is prevented rather than resolved, which is what buys the absence of any merge, transform or "yours or theirs?" dialog. A claim is a **heartbeat, not a flag**: it expires after 30s and beats every 10, because a lock that outlives its holder would leave a shut laptop holding a hymn hostage on the one evening everyone is in a room to work. Held per **field path**, so a lock **crosses surfaces** — a hymn claimed in the [[Planning view]] is locked on the Order of Service page too. In **Shepherding** a box is one [[Care List]] cell, one question on a [[Form Document]], one heading, paragraph, list item or table cell in an [[Elder Document]], one whole [[Person Panel]], a document's title, and the open note, [[Task]] or person-details editor. Quick clicks — Care List columns, a [[Filtered View]] — are not boxes; they just update live. Unlike the Order of Service, **a Shepherding hold lets go after a minute without typing**, because elders are not in one room to ask ([ADR 0062](docs/adr/0062-a-shepherding-document-is-held-paragraph-by-paragraph.md)). The assistant (the MCP) is refused at a held box too, and told who holds it ([ADR 0063](docs/adr/0063-the-assistant-is-refused-at-a-held-box.md)).

**Presence**:
Who else is on this page, drawn as a row of faces. Lives at `presence/{uid}` — one document per editor, named after them, which is what lets the rules say *write your own and nobody else's* and makes the [[Box lock]] enforceable rather than merely agreed. Scoped to **surface and page**: "also here" must mean *here*, or a man reading the Services page appears to be sitting on a Sunday he never opened, and a badge that is wrong is worse than one that is absent. ⚠ **Presence may remove a lock; it may never remove an editor.** It starts late, after sign-in, and both pages once started it from inside the handler that grants editing — so a throw inside it turned the application read-only and looked like a styling bug. Starting it cannot throw at its caller, editing rights are granted *before* it starts, and with it not running every box simply opens. **Shepherding keeps its own**, at `shepherding_presence/{uid}`, readable by elders only: a Shepherding claim says *which person* an elder is looking at, which an editor who is not an elder has no business learning (MS-477). A page has **one** presence, shared by every editor on it — a profile and the Tasks tab inside it claim through the same one, because a person has one record and two would overwrite each other.

**Live updating**:
The **Services** page, the **Order of Service**, the **Shepherding Profile**, **Tasks & Reminders** and the [[Care List]] (its cells, title, columns, and who is on it) listen to Firestore for as long as they are open, rather than reading once on load. Inside the phone app a listen stream that stays silent for five seconds is replaced by re-reading every few seconds — the page cannot tell the difference, and a stream the WebView refuses no longer leaves it blank. A change to a field nobody on this screen has touched is simply adopted; a field this editor has changed is left alone until it saves — which needs no merge and asks nobody a question, because the only case that could need a decision is the one the [[Box lock]] prevents. An adopted value moves the **saved snapshot** with it, or the next save reads it as a local edit and writes it straight back, turning something received into something claimed. A snapshot landing mid-edit never rewrites the box under somebody's hands: every inline editor hides its cell and puts an input in its place, so **a hidden cell is somebody editing** — a signal that cannot drift out of step with what is on screen. A [[Care List]] saves **one field per cell**, and a column change is one change applied to the latest column list, so two elders' cells and column edits never undo each other; a row that leaves the list's filter while somebody is typing in it stays until they leave the cell.
...
- **A signed-out person never touches Firestore.** One Cloud Function hands out the questions and takes the answer, and `firestore.rules` gains nothing — the public path is a door with somebody standing in it, not a hole in the wall. Public submissions pass an invisible bot check (App Check). **Today that check is in monitor** (MS-508): the answering page collects a token; `publicForm` logs missing or invalid ones and does not refuse them. Enforce is a documented, Atlas-escalated param flip (`PUBLIC_FORM_APP_CHECK_MODE`), not the committed default — turning it on in one step already locked the church out of its forms. Break-glass: `docs/ops/ms-508-app-check-break-glass.md`. The MS-364 rate limit is complementary, not a replacement.
...
### Elders only
Separately from the [[Answering rung]], and above it: whether anybody below an elder may **see** the form at all — in the [[Forms library]], in the builder, and on the [[Responses tab]]. The rung says who may *answer*; `elder` there still leaves the template, its questions and every answer open to any editor, because the library and the Responses tab are the editor ladder. So this is the narrower thing, stored as `elderOnly` on the [[Form Template]].
- **Only an elder or a super admin may switch it on — and only they may switch it off.** An editor who could clear it could read the lot, so the rule refuses an editor whose write would leave *or* arrive at an elder-only form. An **admin is not an elder** here, the same way `isElder()` has always read.
- **It moves the [[Answering rung]] to `elder` with it**, forced in the model and refused again at the door. A form nobody below an elder may open, answerable by members, is a record that contradicts itself — and the link still works whatever the library shows. Being served an elder-only form you may not see answers **"there is no form here"**, not "not open to you": its existence is one of the things being kept.
- **The flag is stamped on every [[Response]] and written into every upload's path.** A rule cannot afford to read the form once per answer ([ADR 0018](docs/adr/0018-event-visibility-is-stamped-on-the-occurrence.md) §5, the same reason [[Event visibility]] is stamped onto each occurrence), and `storage.rules` cannot see a Firestore field at all — so an elder-only form's files live under `form_uploads_elder/`, chosen in one place (`uploadPath`). Whoever may read an answer may read what came with it, and shutting one has to shut the other.
- ⚠ **Every form and every answer carries the flag, false included.** A rule that narrows per document does not narrow a **query**: Firestore refuses the whole query unless it can see every row it could return is allowed, so a reader below elder asks for `elderOnly == false` by name — and that cannot match a document where the field is absent. Saves write it now; `scripts/backfill-form-elder-only.js` stamped the ones that pre-date it, and it must be run **before** the rules deploy or the library goes blank for every editor who is not an elder.
- **A [[Form Document]] template is not forced shut**, though the documents it makes are elder-only already by living among the [[Elder Document]]s. Forcing it would quietly take every existing document template away from the editors using it.
_Avoid_: private form, hidden form, elder rung (that is the [[Answering rung]]'s top value, which is a different question)
...
**Element**:
One real HTML element on a page: a **box** (a div that holds others), a run of **text** (p, a heading, a span — one string with CSS; bold words are more elements, not rich text), or an **image**. Carries its inline CSS, an optional name, and optionally a [[Binding]] or a [[Repeat]]. The **element panel** on the editor's left edits whatever is selected in plain words — size, position, spacing, border, background, font, alignment, how children are arranged — and a **custom CSS** box takes the rest. A parent in the **Elements** tree folds its children underneath it so a deep page is not a long list; picking an element on the canvas opens the folds that hide it. Who an [[Repeat]] stands for is the **query** in the [[Data drawer]], not this panel. The **code view** shows the page's elements as HTML and its stylesheet as CSS; an edit there is parsed back into elements, ids and wires intact, and refused by line when the markup is broken.
_Avoid_: node (the code's word, not the church's), component (the ADR 0008 word), widget

**Sizing** (**Hug**, **Fixed**, **Fill**):
How an [[Element]] takes its width and its height, said the way the layout tools people already use say it: **Hug** is as big as what is inside it, **Fixed** is the number you typed, **Fill** is as big as the space its parent gives it — for a box straight on the page, margin to margin. The mode is never stored; it is read back off the element's own CSS (`printable-core.js`), so a page written by hand in the code view reads the same as one clicked together in the panel. What Fill has to write depends on what the parent does with its children — a share of a row, a stretch across a column, an auto width in a plain box — and **Align self** says how an element sits across that. Padding, margin and border width are each shown at whichever **fidelity** they need: all four sides the same, the two pairs, or each side on its own, the same three the page's margins offer.
_Avoid_: auto layout (Figma's word, and it names the parent's side of it), stretch (that is one align, not a size), hug contents

**Data drawer**:
The editor's right panel. It opens empty. Select an [[Element]] and the drawer offers a job: a box with no [[Repeat]] gets **Make this element iterated**, which opens the **query** builder. The query builder is how you pull up every **iterable** list the website can tell a Printable — People, Households, the Sunday lists, event dates, form answers — picked from a **dropdown** grouped as the catalog used to group them, then the filters this viewer may use and how many rows match today. **Not all data could be pulled** folds; the count stays on the heading. A **single** (this Sunday, who holds a role, Sunday booklet text) is not a list and is not in the builder. **Sundays** counts Sundays from a Sunday — last, this, next, or one picked — and lists every Sunday in the count whether or not anybody has written it yet, with any service written on a weekday inside the count as well; a Sunday nobody has written reads **TBA**, or whatever the editor types instead. So a preaching schedule is one iterated row over five Sundays, and a list of one Sunday from this Sunday reads this Sunday. **Announcements of a Sunday** is one row per announcement handed out that Sunday — the typed ones first, then printed [[Event Announcement]] lines — so a card can repeat the way the old guide listed them; **Sunday booklet text** still joins the same items into one field. **Event dates** count weeks from a Sunday the same way, or the **next few dates** of something (a monthly meeting is not missed because the window was a fortnight), and an editor may add who is down for a Role on each date — a rota; a member is never offered it. An unbound box inside an iterated card whose parent has a **related list** gets **Make this a sub-iteration** instead — **Children** is of a [[Household]] card, so each home lists its own children, not a second directory. A child that already has a [[Binding]] is a field of the parent row and is not offered a nested list; a text or image cannot stand for a row. The catalog (`printable-data-core.js`) is still the first half of the permission boundary and holds no elder-only *source*; a *filter* on a source may sit higher (Membership stage, inactive people) and the query builder hides it from anyone who may not use it. A stored query still runs. `firestore.rules` is the second half. Those children can be kept to members or to non-members. Households themselves can be kept to those **with children**, or to those with a **non-member child** (or a member child). An elder may publish a Printable whose query another editor could not rebuild.
_Avoid_: variables panel, connectors (the brief's word; a field is what you drag)

**Sunday booklet text**:
The weekly typed values that belong to a Sunday, not to a Printable's box tree: prayer-country facts, Mosaic Kids lesson, and announcements (`services/{date}.typedContent`). An editor types them once — in the Printable data drawer, or they are read through from that week's Service Guide if the store is still empty. Every Printable bound to that Sunday reads the same fields. Sunday A's values never appear on Sunday B. The announcements a Printable reads also include the printed [[Event Announcement]] lines for that Sunday, read when the booklet is opened and not stored with the typed values.
...
A box that stands for one row of a list and is drawn once per row — the [[Data drawer]]'s **Make this element iterated** (or right-click the same), then build the **query** there (which list, which filters this viewer may use). A box inside an iterated card can query a **related list** of that row (the children of this household) instead of a second flat directory — the drawer's **Make this a sub-iteration**, only when that box has no [[Binding]] yet. Layout of the copies (down or across, per line, gap, a cap per page) stays on the element panel. The query only offers sources and filters the signed-in viewer may use; a stored query an elder built still runs for everyone else. The first copy on the first page keeps the element's own id and is the one you edit — the seed. The rest carry the row number after a tilde and are drawn, never edited, including the first card on a continuation page. **Overflow** is either **Clip** (the list stops at the margin) or **Makes a new page**: the list stops before the margin and continues on **real pages** — each stored, each its own tree in the Elements panel, each editable (its own page number, header, footer). New pages start as a copy of the page the list started on, or a page chosen instead; after that they keep the design you give them. The rows stay live: the same list is sliced across the chain as today's data grows or shrinks. Extra pages stay if the list shrinks; delete them if you do not want them. How many rows fit is measured in the browser and decided by a pure bisection (`printable-render-core.js`). The Sunday hymns list is one row per **sheet-music page** (MS-481), so a booklet page with **Makes a new page** and one copy per page places every page of a multi-page hymn, in order. A missing image stays blank; nothing is invented.
...
**Linked Printable**:
A Printable an [[Event]] series carries (`events/{id}.printables`), so every date of it offers the Printable on its Files tab: open in the editor, view, **Members may view**, unlink, and **File a PDF snapshot**. The snapshot renders every page — overflow continuation pages included — with vendored html2canvas and jsPDF, and files it as an ordinary [[Event Attachment]] on that date, fetched and never linked ([ADR 0046](docs/adr/0046-an-event-attachment-is-fetched-never-linked.md)). It is the one frozen copy in a live system ([ADR 0057](docs/adr/0057-a-printable-reads-live-data-the-snapshot-on-an-event-is-the-frozen-copy.md)); unlinking keeps both the Printable and the snapshot.
...
- **"Announcement"** is two records that share a page. The weekly announcements an editor types onto one Sunday stay that Sunday's own words. An [[Event Announcement]] belongs to an Event, is printed or told on a schedule, and is read beside the weekly ones — never stored as one of them.
