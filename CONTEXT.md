...
**Link Request**:
A User's own request to become a [[Linked User]] — the front door to the directory for anyone who signs up. Stored at `link_requests/{uid}`, one live request per account by construction (the document id *is* the requesting User's uid). Comes in exactly two **Kinds**: a **Match Request** names an existing Person and says "that is me", and a **New Record Request** carries proposed details and says "I am not in the directory yet". A request is `pending` until an **Approver** — an editor, elder, admin or super admin, the same set as the directory's Edit Mode — resolves it. **Approving** a Match Request writes both sides of the link; approving a New Record Request creates the Person first and then links it. An Approver may also **redirect** a New Record Request onto an existing Person they recognise, which is how a duplicate record is prevented rather than merged later. **Declining** closes the request with an optional reason shown to the requester, who may then withdraw it and ask again. A Person created this way starts at the **Visitor** [[Membership Stage]] — nobody self-declares onto the Membership Track. Approval is a privileged act performed by the `resolveLinkRequest` callable, never from a browser: it writes `users/{uid}.personId`, and the `users` collection stays admin-only. Decided in [ADR 0025](docs/adr/0025-link-requests-are-self-raised-and-editor-resolved.md).
_Avoid_: Invite (nothing is sent to the person; they start it), claim, application, join request
