...
- **One date decides WHO; the Event decides WHAT.** A **Sunday** is the same rule taken furthest: its pattern is settled by definition, its visibility by rule, and its liturgy belongs to the order of service — so one Sunday offers no pattern controls and no *Skip this one*, only who is serving and its own [[One-off Role]]s.
   ...
   - **An [[Event Announcement]] is not that exception.** The standing words belong to the event. A date may show them and may offer the way through to where they are edited. It may not add, change, or remove one.
...
### Event Announcement
A title and a short piece of prose that belongs to an event — the [[Event series]], or the one-off whose [[Event occurrence]] is the whole event. An event may carry several, in the order they were added. Each one is announced in exactly one way: printed in the Sunday service guide a whole number of weeks ahead, or told to people as a [[Notification]]. Doing both is two announcements. The editor does not choose push or text (ADR 0036).
- **A date of a repeating event reads it and cannot change it.** The words are edited where the event's name and description already live. One date shows them and links through to that place.
- **Who would be told is tags, and every tag chosen.** The people are those who carry all of the chosen [[Shepherding Tag]]s. No tags chosen cannot be saved: an empty filter is not everyone. Somebody Inactive is included only when the Inactive tag is one of the tags chosen.
- **A reader who is not an editor sees the title and the prose.** Not the way, the schedule, the tags, or the names. Who may read the words is who may already read the event.
_Avoid_: announcement (unqualified — the Sunday service guide already has weekly typed announcements, which are words on one Sunday and stay there), blast, reminder, text (the channel; this is a Notification)
