...
**Pastoral Assistant**:
A grant on a User account (`users.pastoralAssistant`), stacked on their [[Permission Level]] — not a Permission Level itself ([ADR 0065](docs/adr/0065-a-pastoral-assistant-is-a-grant-on-the-account.md)). A Pastoral Assistant reads everything an Elder reads (including the editor-rung things an elder sees because an elder is an editor) and writes the record: Elder Documents of every kind, Folders, Shepherding Notes, and [[Task]]s, with the same freedom elders have over those. They make no elder decision and are never counted as an Elder: no [[Elder Tag]], not in elder pickers, no [[Elder Digest]], and they cannot set [[Shepherding Status]], change tags, set [[Elder Assignment]], add an [[Explanation]], manage [[Filtered View]]s or Relationships, write or text [[Prayer Request]]s, lock guidance, shut a form to elders, or answer an elder-rung form. Only an admin or super admin can grant it.
