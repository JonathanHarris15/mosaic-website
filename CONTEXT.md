...
**It steps Sunday to Sunday.** Arrows beside the date open the next or previous Sunday without going back to [[Services]] — the guide party works forward through weeks nobody has touched yet, so an untouched Sunday is a destination, not something to skip. The far ends (the project's first Sunday, two years out) grey the arrow rather than failing quietly. Stepping leaves **no trail**: the `‹ Services` link is the way out and one press of the browser's back means the same thing, never *undo my last arrow*. On a phone the arrows are not there yet: the app shell draws its own header, which still reads "Service Editor" and names no Sunday at all (MS-310).
