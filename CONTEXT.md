...
**It saves one answer at a time, and updates live** (MS-431). Each answer is its own field and a save writes only the answers that changed — the whole answer is the unit, so a picked person's id and name, or a select-all list, travel together. Two elders on two questions never overwrite each other; each question that asks something, and the title, is a [[Box lock|box]]. Only the save that changed who a [[Personal shepherding document]] is about re-files it — the page's or the assistant's — and every open page takes in where it is now filed.
