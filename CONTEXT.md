...
**Block**:
One heading, paragraph, list, list item, table, row, cell, picture, rule or [[Person Panel]] of an [[Elder Document]]'s Note Body, stored as its own field `blocks.<id>` (ADR-0065). Its id lasts: typing, Enter and paste never hand one id to two blocks. It knows the block it sits inside (`parent`) and its place among its siblings (`order`, a key that can always fit another between two). A save writes only the Blocks that changed, so two elders in two paragraphs never undo each other. A paragraph, list item, cell or Person Panel is a box in the [[Box lock]]; a list, table or row is only structure.
