...
- **Roles come from two levels.** The [[Event series]]' `roleSlugs` apply to *every* date of it, minus the liturgical ones; `occurrenceRoleSlugs` are added to *this date alone*. Both draw as fillable cards with the full Assignment state machine; a Role inherited from the series is not removable from one date, because doing so would either silently change every date or silently change none.
...
- **A Sunday occurrence is `services/{date}`**, which keeps its liturgical roles as the hardwired fields the Service Guide prints. Assignments sit *alongside* those fields and never over them — a Sunday's **Servant** Roles are filled on its Event page like any other date, and a liturgical Role is never drawn as a fillable card there.
