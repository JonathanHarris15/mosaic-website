...
```
hosting,functions:publicForm,functions:onAttendanceCreated,functions:syncAccountRankToPerson,functions:sendPrayerRequestNow,functions:mcp,functions:adminNotificationOverview,functions:adminPushDevices,functions:adminRevokePushToken,functions:adminSendSelfPushTest,firestore:rules,firestore:indexes
```
...
| `functions:adminNotificationOverview` | Push admin tab — flow + trigger registry (MS-682). Without this target the tab cannot load overview data before Hosting ships. |
| `functions:adminPushDevices` | Masked push token inventory for admins (MS-682). Owner-only token rules stay; admins read via Admin SDK here. |
| `functions:adminRevokePushToken` | Admin revoke one device token (MS-682). |
| `functions:adminSendSelfPushTest` | Admin test push to the signed-in admin's own devices only (MS-682). |
| `firestore:rules` | Live `firestore.rules` (MS-565). Without this target a Hosting merge that needs a rules hole (MS-530 / #69 Pastoral Assistant `lastNoteAt`) ships writers against the old rules. |
| `firestore:indexes` | Composite indexes for push admin queries (`notifications` history, `push_tokens` collection group). Deploy before Hosting that queries them. |
...
`targets=hosting,functions:publicForm,functions:onAttendanceCreated,functions:syncAccountRankToPerson,functions:sendPrayerRequestNow,functions:mcp,functions:adminNotificationOverview,functions:adminPushDevices,functions:adminRevokePushToken,functions:adminSendSelfPushTest,firestore:rules,firestore:indexes`
