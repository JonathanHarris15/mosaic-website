...
say ""
say "The answering page now sets FIREBASE_APPCHECK_DEBUG_TOKEN=true BEFORE it"
say "calls activate() whenever the origin cannot attest (localhost). Without"
say "that flag the SDK never prints a UUID and never exchanges one — which is"
say "part of why turning enforce on once hung local and phone-shell submits."
say ""
...
note "The phone app does not use a debug token for members. Capacitor serves"
note "the fill-in page from capacitor://localhost, which reCAPTCHA cannot"
note "attest, so the page hops onto the hosted HTTPS origin (liveOrigin in"
note "public/app-check-config.js) and mints a real token there. Same door."
