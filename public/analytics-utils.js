// Analytics Utils — turning a free-text scripture reference into the
// book/chapter/verse(s) it actually names.
//
// Shared between the Analytics page's Bible heat map and the verse picker's
// own heat-map coloring (public/verse-picker.js) — both need the same
// answer to "what does this reference actually cover", so it lives once.
//
// Loaded as a classic <script> before both callers. analytics.js is this
// codebase's one sanctioned ES module (CLAUDE.md) — it reads
// window.AnalyticsUtils rather than importing this file, so this file stays
// a plain classic script like everything else and both callers agree.
(function (global) {
    'use strict';
     ...
     */
    function parseBibleReference(ref, bibleData) {
        if (!ref) return [];
    ...
    const AnalyticsUtils = { parseBibleReference };

    if (typeof module !== 'undefined' && module.exports) {
        module.exports = AnalyticsUtils;
    }
    if (global) {
        global.AnalyticsUtils = AnalyticsUtils;
    }
